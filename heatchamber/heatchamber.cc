...
ISR(TIMER2_OVF_vect)
{
	...
	++t2;
	if (ac_crossings < CALIBRATION_CYCLES && t2 > t2_max) {
		t2_max = t2;
	}

	// t2 == 0 is handled in PCINT1_vect
	if (t2 == trigger) {
		DimmerOut::set();
	}
}
...
int main(void)
{
	...
	for (;;) {
		...
		{
			// display current detected
			Display::character(124, 0, '~', glyphs_huge);
